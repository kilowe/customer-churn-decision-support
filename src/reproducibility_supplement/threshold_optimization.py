...
def build_threshold_grid() -> pd.DataFrame:
    ...
    thresholds = np.round(np.arange(THRESHOLD_START, THRESHOLD_END + 1e-9, THRESHOLD_STEP), 2)

    rows: list[dict] = []
    for model_name, score_col in [("baseline", "baseline_proba"), ("balanced", "balanced_proba")]:
...
def select_max_f1(df_model: pd.DataFrame) -> dict:
    ...
    ranked = df_model.sort_values(by=["f1", "recall", "threshold"], ascending=[False, False, True])
...
def select_cost_based(df_model: pd.DataFrame, scenario_name: str, cfn: int, cfp: int) -> dict:
